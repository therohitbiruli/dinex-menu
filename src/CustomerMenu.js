...
const CustomerMenu = () => {
    ...
    const placeOrder = async () => {
        ...
        try {
            await setDoc(doc(db, "orders", orderId), { tableId: tableId, items: order, status: "new", createdAt: new Date() });
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="bg-white shadow-sm sticky top-0 z-10">
                <div className="max-w-2xl mx-auto px-4 py-4"><div className="text-center"><h1 className="text-xl font-bold text-gray-800">{selectedRestaurant?.name || "Restaurant Menu"}</h1>{selectedRestaurant?.description && (<p className="text-sm text-gray-600">{selectedRestaurant.description}</p>)}</div></div>
            </div>
            <div className="max-w-2xl mx-auto px-4 py-6 pb-24">
                <div className="bg-white rounded-lg p-4 shadow-sm border mb-6">
                    <div className="relative mb-4"><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" /><input type="text" placeholder="Search menu items..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg" /></div>
                ...
                {filteredItems.length === 0 ? (<div className="text-center py-12 text-gray-500"><Menu className="w-12 h-12 mx-auto text-gray-300 mb-4" /><p className="text-lg font-medium mb-2">No menu items found</p><p className="text-sm">The menu is currently empty. Please check back later.</p></div>) : (categories.map(category => { const categoryItems = filteredItems.filter(item => (item.category || "Uncategorized") === category); if (categoryItems.length === 0) return null; return (<div key={category} className="mb-8"><h2 className="text-xl font-bold text-gray-800 mb-4 px-4">{category}</h2><div className="space-y-4">{categoryItems.map(item => (<div key={item.id} className="bg-white rounded-lg shadow-sm p-4 border"><div className="flex gap-4">{item.imageUrl && <img src={item.imageUrl} alt={item.name} className="w-20 h-20 rounded-lg object-cover cursor-pointer flex-shrink-0" onClick={() => setViewingImage(item.imageUrl)}/>}<div className="flex-1 min-w-0"><div className="flex items-start justify-between mb-2"><h3 className="font-semibold text-gray-800 text-lg">{item.name}</h3><div className="text-xl font-bold text-gray-800">₹{item.price}</div></div><div className="flex flex-wrap items-center gap-2 mb-2">{item.isVeg ? <span className="text-green-600 text-sm">🟢</span> : <span className="text-red-600 text-sm">🔴</span>}{item.popular && <span className="text-xs bg-orange-100 text-orange-800 px-2 py-1 rounded-full">⭐ Popular</span>}{item.chefSpecial && <span className="text-xs bg-purple-100 text-purple-800 px-2 py-1 rounded-full">👨‍🍳 Chef's Special</span>}</div><p className="text-gray-600 text-sm leading-relaxed">{item.description}</p><div className="mt-3"><button onClick={() => addToOrder(item)} disabled={tableStatus === 'ordering'} className="bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg text-sm hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed">Add to Order</button></div></div></div></div>))}</div></div>);}))}
                <div className="text-center py-8 border-t mt-8"><p className="text-sm text-gray-500">Powered by <span className="font-semibold">DineX</span> - Quintex Digital Solutions</p></div>
            </div>
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-gray-50/70 backdrop-blur-sm border-t">{tableStatus === 'ordering' ? (<div className="bg-yellow-100 border-yellow-500 text-yellow-700 p-4 rounded-lg shadow-lg text-center"><p className="font-bold">Your order is being prepared.</p><p className="text-sm">You can place a new order after this one has been served.</p></div>) : order.length > 0 ? (<div className="bg-white p-4 rounded-xl shadow-lg border flex items-center justify-between"><div><p className="font-bold text-lg">{order.length} items in your order</p><p className="text-sm text-gray-600">Total: ₹{order.reduce((total, item) => total + item.price, 0)}</p></div><button onClick={placeOrder} className="bg-green-600 text-white font-bold px-6 py-3 rounded-lg hover:bg-green-700">Place Order</button></div>) : null}</div>
